...
function showCategory(category) {
    if (category == 'phones') {
        for (let i = 0; i < products.phones.length; i++){
            getProductMarkup(products.phones, products.phones[i].id, products.phones[i].name, products.phones[i].price, products.phones[i].imageUrl);
        }
    } else if (category == 'tablets') {
        for (let i = 0; i < products.tablets.length; i++){
            getProductMarkup(products.tablets, products.tablets[i].id, products.tablets[i].name, products.tablets[i].price, products.tablets[i].imageUrl);
        }
    } else if (category == 'tv') {
        for (let i = 0; i < products.tv.length; i++){
            getProductMarkup(products.tv, products.tv[i].id, products.tv[i].name, products.tv[i].price, products.tv[i].imageUrl);
        }
    }
// долго старался найти информацию, как по строчному наименованию найти объект - не нашел
}
 ...
 * в верху этого файла.
 */
function getProductMarkup(product, productId, productName, productPrice, productImgUrl) {
    return div.insertAdjacentHTML('afterbegin',`<div>${productName}</div><img src="${productImgUrl}" alt=""><div>${productPrice}</div><a href="https://example.com/producs/${productId}">Подробнее</a>`);
}